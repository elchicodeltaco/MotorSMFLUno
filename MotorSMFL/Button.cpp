...
namespace GUI
{
	Button::Button(const FontHolder& fonts, const TextureHolder& textures)
		...
		, mIsToggle(false)
	{
		mSprite.setTexture(mNormalTexture);//asignamos imagen al botón
	...
	{
		Component::activate();
		//si estamos en toggle, entonces mostamos que el botón está apretado
		if (mIsToggle)
			mSprite.setTexture(mPressedTexture);
		//Si  tiene una funcion, hacemos callback
		if (mCallback)
			mCallback();
		//Si no estamos en toggle, desactivamos el botón.
		if (!mIsToggle)
			deactivate();
	}
	...
	void Button::draw(sf::RenderTarget& target, sf::RenderStates states)const
	{
		...
		target.draw(mSprite, states); //primero dibujamos el botón
